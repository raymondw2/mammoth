...
class SequentialMWM(ContinualDataset):
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        self.envs = {}
        list_of_goal = [[-6, -5], [-4, 4], [9, 6], [10, 1], [-4, -8]]
        for i in np.arange(self.N_TASKS):
            ...
            self.envs[env_name].place_goal_location(list_of_goal[i])
...
class SquareMaze():
    ...
    def get_random_observation(self,):
        ...
        obs = torch.Tensor(self.get_vision()).reshape(1,1000).unsqueeze(dim = 1)
        return obs, torch.tensor(self.find_optimal_move()).long()
